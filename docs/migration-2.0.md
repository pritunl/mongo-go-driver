...
The `context.Context` parameter has been removed from `mongo.Connect()` because the [deployment connector](https://github.com/pritunl/mongo-go-driver/blob/a76687682f080c9612295b646a00650d00dd16e1/x/mongo/driver/driver.go#L91) doesn’t accept a context, meaning that the context passed to `mongo.Connect()` in previous versions didn't serve a purpose.
...
The `MarshalBSONValue` method of the [ValueMarshaler](https://pkg.go.dev/github.com/pritunl/mongo-go-driver/v2/bson#ValueMarshaler) interface now returns a `byte` value representing the [BSON type](https://pkg.go.dev/github.com/pritunl/mongo-go-driver/v2/bson#Type). That allows external packages to implement the `ValueMarshaler` interface without having to import the `bson` package. Convert a returned `byte` value to [bson.Type](https://pkg.go.dev/github.com/pritunl/mongo-go-driver/v2/bson#Type) to compare with the BSON type constants. For example:
...
The `UnmarshalBSONValue` method of the [ValueUnmarshaler](https://pkg.go.dev/github.com/pritunl/mongo-go-driver/v2/bson#ValueUnmarshaler) interface now accepts a `byte` value representing the [BSON type](https://pkg.go.dev/github.com/pritunl/mongo-go-driver/v2/bson#Type) for the first argument. That allows packages to implement `ValueUnmarshaler` without having to import the `bson` package. For example:
